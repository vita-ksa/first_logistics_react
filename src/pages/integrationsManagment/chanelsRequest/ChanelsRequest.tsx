...
export const ChanelsRequest = () => {
  ...
  const data = useSelector(
    (state: any) => state?.deliveryShopRequestState?.entities?.requests || []
  )
  const reloadData = useSelector((state: any) => state?.approveShopRequestState?.entities || [])
  ...
  const fetchData = useCallback(async ({search}: any) => {
    const {payload} = await dispatch(
      integrationsAPI.getDeliveryShopRequest()({
