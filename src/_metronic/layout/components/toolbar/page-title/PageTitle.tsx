  ...
  return (
    ...
    >
      ...
      {config.app?.pageTitle?.display && pageTitle && (
        ...
        >
          {pageBreadcrumbs?.[0]?.title || pageTitle}
      ...
      {pageBreadcrumbs &&
        ...
        config.app?.pageTitle?.breadCrumb && (
          <>
            ...
            <ul className='mt-1 breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0'>
              {Array.from(pageBreadcrumbs).map((item, index) => {
                return (item?.title && !item.isSeparator)? (
                  <li
                  ...
                  >
                    {!item.isSeparator ? (
                      <Link className='text-primary text-hover-primary' to={item.path} state={item?.state}>
                        {item?.title} -
                      </Link>
                    ) : (
